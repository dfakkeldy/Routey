...
*Generated from 9 commits merged during the week.*
...
### Jul 1, 2026 - Internal TestFlight and docs sync

The nightly train now has the iPhone app shell for route import/editing,
predictive search, Snap-to-Add camera OCR, and the first Today's Run drive-loop
UI. A manual `nightly` release-train run built Routey `0.1 (4)`, waited for App
Store Connect processing, and distributed it to internal TestFlight testers.

The release docs now treat that as upload proof only. V1.0 still needs the
remaining visible delivery workflow, Production CloudKit validation, final App
Store metadata, screenshots, privacy/compliance answers, pricing decisions, and
a device smoke pass with invented placeholder data.
